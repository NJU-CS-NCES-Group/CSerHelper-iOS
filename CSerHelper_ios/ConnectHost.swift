...
class ConnectService: NSObject {
    ...
    init(id:String,number:String,password:String,classid:String?) {
        self.id = id
        self.number = number
        self.password = password
    ...
    func startService6(completionHandler:@escaping (_ data:Data?,_ response:URLResponse?,_ error:Error?) -> Void){
        
        let str = desHost + "id=\(id)&idnumber=\(number)&password=\(password)&classid=\(classid!)"
        let url: NSURL = NSURL(string:str)!
        //2.获得会话对象
        let session: URLSession = URLSession.shared
...
struct JSONcoder5 : Codable {
    let classid:String
    let fullname:String
}
struct JSONcoder6 : Codable {
    let id:String
    let root:String
    let name:String
    let created:String
